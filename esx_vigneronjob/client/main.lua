...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Véhicule pour apprenti',  value = 'bison3'},
			{label = 'Véhicule pour employé(e)',  value = 'rebel2'},
			{label = 'Livraison', value = 'speedo'},
			{label = 'patron', value = 'gt500'}
		}
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBillneo', GetPlayerServerId(closestPlayer), 'society_vigne', 'Vigneron', amount)
								end)
							end
						end
					end,
