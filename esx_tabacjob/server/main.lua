...
AddEventHandler('esx_tabac:pedBuyCig', function()
  ...
  local resellChances = {}
  local cigTypeMagic  = math.random(30, 40)
  local chosenCig     = nil
  local prices        = nil
...
ESX.RegisterUsableItem('malbora', function(source)
    ...
    TriggerClientEvent('esx:showNotification', source, 'Vous avez fumé ~g~1x ~b~Redwood')
...
ESX.RegisterUsableItem('gitanes', function(source)
    ...
    TriggerClientEvent('esx_tabac:onSmokeCig', source)
    TriggerClientEvent('esx:showNotification', source, 'Vous avez fumé ~g~1x ~b~Cigar')
      ...
      elseif xPlayer.getInventoryItem('gitanes').count <= 0 and malbora == 0 then
        ...
        if (gitanes == 1) then
          SetTimeout(1100, function()
            local money = math.random(30,33)
            local argent = math.random(30,33)
            xPlayer.removeInventoryItem('gitanes', 1)
            local societyAccount = nil
